package com.liudengjian.imageviewer.view.util;
...
import com.liudengjian.imageviewer.evaluator.MatrixEvaluator;
import com.liudengjian.imageviewer.evaluator.RectFEvaluator;
import com.liudengjian.imageviewer.listener.OnViewerformListener;
import com.liudengjian.imageviewer.util.Compat;
import com.liudengjian.imageviewer.util.Util;
import com.liudengjian.imageviewer.view.ViewerImageView;



/**
 * Android的2D变形（包括缩放，扭曲，平移，旋转等）可以通过Matrix来实现
 */
...
public class TransformAttacher {
    ...
    public TransformAttacher(ViewerImageView imageView) {
    ...
    public void showThumb(boolean needTrans) {
        if (needTrans)
    ...
    public void showImage(boolean needTrans) {
        if (needTrans) {
    ...
    public void changeState(TransState state) {
        if (running) return;
        running = true;
    ...
    private void runTransform() {
        ...
        animation.setListener(new OnViewerformListener() {
            @Override
...
//            if (rectF == null) return null;
    ...
    class TransformAnimation implements Runnable {
        ...
        void setListener(OnViewerformListener listener) {
            this.listener = listener;
        }
        ...
        public void run() {
            ...
            if (t < 1f) {
                ...
                isRunning = false;
                if (listener != null) listener.viewerformEnd();
            }
        }
        ...
        void start() {
            isRunning = true;
            if (listener != null) listener.viewerformStart();
            Compat.postOnAnimation(imageView, this);
        }
    }
}
